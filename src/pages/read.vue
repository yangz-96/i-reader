<template>
    <section :style="{'background-color': '#f7f2e8','height':this.clientHeight+'px'}">
        <!--<transition name="van-slide-down">
            <van-nav-bar title="目录" left-arrow z-index="9999" v-show="show"/>
        </transition>-->
        <div class='popContainer'>
            <div style="width: 30%;height: 100%;float: left;" @click="previousPage"></div>
            <div style="width: 40%;height: 100%;float: left;" @click="show = !show"></div>
            <div style="width: 30%;height: 100%;float: right;" @click="nextPage"></div>
        </div>

        <div id="content" ref="content" class="content" :style="{'font-size':'15px','line-height':'26px',}"  v-html="pageList[page]">
        ...
        <van-popup v-model="show" position="top" :overlay="false">
            <van-nav-bar :title="chapter.title" left-arrow z-index="9999"/>
        </van-popup>
    </section>
</template>
...
<script>
    ...
    export default {
        ...
        mounted(){
            ...
            document.body.style.height = this.clientHeight;

            let content = this.chapter.body;

            let strings = content.split("\n");
            this.contentWidth = this.clientWidth-40;
            this.contentHeight = this.clientHeight-40;
            console.log("内容宽度：" + (this.clientWidth-20));
            console.log("内容高度：" + (this.clientHeight-20));
            let count = 0;
            let pageText = '';
            for(let i=0;i < strings.length;i++){
                console.log(strings[i]);
                //console.log((strings[i].length * 15)/(this.clientWidth-20));
                //console.log(parseInt((strings[i].length * 15)/(this.clientWidth-20))+1);
                //本段的行数
                let rowNum = parseInt((strings[i].length * 15)/(this.clientWidth-40))+1;
                //总行数
                //count += rowNum;
                if( (count + rowNum) * 26 > this.contentHeight ){
                    //计算剩余行数
                    let tempHeight = this.contentHeight - count * 26;
                    console.log(tempHeight/26);
                    let textlength = parseInt(tempHeight/26) * this.contentWidth;
                    console.log(textlength);
                    let size = parseInt(textlength/15);
                    pageText += strings[i].substr(0,size + 3);
                    this.content += strings[i].substr(0,size + 3);
                    this.pageList.push(pageText);
                    pageText = strings[i].substring(size+3,strings[i].length) + "\n";
                    count = 0;
                }else {
                    count += rowNum;
                    pageText += strings[i] + "\n";
